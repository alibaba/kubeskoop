...
func (s *aliyunInfraShim) NodeToNode(src *v1.Node, oif string, dst *v1.Node, packet *model2.Packet) ([]model2.Suspicion, error) {
...
func (s *aliyunInfraShim) NodeToExternal(src *v1.Node, oif string, packet *model2.Packet) ([]model2.Suspicion, error) {
	srcID, err := s.getECSFromNode(src)
	if err != nil {
		return nil, err
	}
